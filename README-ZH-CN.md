...
**VS Code 兼容扩展** — 通过 VS Code 兼容扩展 API 采集当前编辑器状态，并托管一个插件本地的 IPC 服务。它不仅可用于 VS Code，也可用于 Cursor、Trae 等支持 VS Code 扩展的编辑器。该 IPC 服务并非公开的 MCP 端点，仅供内置的 stdio adapter 使用。扩展会在用户的应用数据目录下写入一份注册文件，使 stdio adapter 能够定位到当前活动的 IPC 端点。
...
**JetBrains 插件** — 面向 IntelliJ Platform IDE 遵循同样的 stdio 优先方向。通过带运行期 token 的插件本地 loopback IPC 暴露编辑器状态，并配置 Agent 通过生成的 launcher 启动内置 JVM `StdioAdapter`。

**Agent 配置** — 所有受支持的 Agent 均以 stdio 方式配置。每个编辑器写入独立的 MCP server 名称，因此多个编辑器插件可以共存：

- VS Code：`editor-context-vscode`
- Visual Studio：`editor-context-visualstudio`
- JetBrains IDE：`editor-context-jetbrains`
- Codex：`[mcp_servers.<server-name>]` 中的 `command` 和 `args`
...
如果用户同时安装了 VS Code、Visual Studio 与 JetBrains 插件，每个编辑器各自拥有自己的 IPC 端点。活动的 adapter 会选择由该编辑器插件登记的端点。
...
![Visual Studio 设置](docs/images/visualstudio-settings.png)

**Visual Studio Code：**

![VS Code 面板](docs/images/vscode-dashboard.png)

**JetBrains：**

![JetBrains 设置](docs/images/jetbrains-settings.png)
...
VS Code 兼容扩展项目位于 `extensions/vscode`。它面向 VS Code，也支持 Cursor、Trae 等可安装 VS Code 扩展的编辑器。
...
该扩展为 JavaScript 实现，只要目标编辑器支持所需的 VS Code 扩展 API，即可跨平台运行。

### JetBrains 插件

JetBrains 插件项目位于 `extensions/jetbrains`，使用 Kotlin 实现，面向 IntelliJ Platform IDE。它面向 JetBrains IDE 2024.3+（`since-build=243`），并输出 Java 21 兼容字节码。

前置条件：

- JDK 21+
- Gradle，或带 Gradle 的 IDE/CI 环境

优先使用统一脚本。若要手动构建 JetBrains 插件：
...
从 `Settings > Plugins > Install Plugin from Disk...` 安装，然后重启 IDE。从 `Tools > ContextToAgent Settings...` 或 `Settings > Tools > ContextToAgent` 打开配置页面。JetBrains 桥接使用与 VS Code、Visual Studio 插件相同的 MCP server 名称和工具协议。
...
- 在 VS Code、Cursor、Trae 或其它兼容编辑器中加载该扩展。
...
- 让 Agent 调用 `editor-context-vscode.get_context`。
- 验证空选区仅返回路径与光标。
- 验证选中文本仅返回所选文本。
...
JetBrains 检查需在 JetBrains IDE sandbox 或已安装 IDE 中运行。确认设置页可打开、生成的 launcher 被写入 Agent 配置、空选区不返回文本、选中文本仅返回实际选中内容、错误高亮最多 50 条。



最后，感谢**LinuxDo**社区的开源环境！！
