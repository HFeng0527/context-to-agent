...
**Agent Configuration** — All supported agents are configured with stdio:
...
The repository ships a single script that packages both plugins:

```powershell
npm run package
```

This runs the verification step, then builds the Visual Studio VSIX and the VS Code VSIX into `artifacts/`. Use the switches to skip either side:
