...
namespace EditorContextBridge2026
{
    public partial class BridgeSettingsDialog : Window
    {
        ...
        private void ConfigureAll_Click(object sender, RoutedEventArgs e) => _ = RunUiActionAsync(async () => { if (MessageBox.Show("Update supported stdio MCP configs?", "Editor Context Bridge", MessageBoxButton.OKCancel) == MessageBoxResult.OK) { _agentConfigService.ConfigureAll(_bridgeClient); await RefreshAsync(); } });
        private void RevokeAll_Click(object sender, RoutedEventArgs e) => _ = RunUiActionAsync(async () => { if (MessageBox.Show("Remove editor-context from managed MCP configs?", "Editor Context Bridge", MessageBoxButton.OKCancel) == MessageBoxResult.OK) { _agentConfigService.RevokeAll(_bridgeClient); await RefreshAsync(); } });
        private void ConfigureOtherAgents_Click(object sender, RoutedEventArgs e) => _ = RunUiActionAsync(async () => { var text = _agentConfigService.ConfigureOtherAgentsText(_bridgeClient); Clipboard.SetText(text); MessageBox.Show(text + Environment.NewLine + Environment.NewLine + "Other agents configuration guide copied to clipboard.", "Editor Context Bridge", MessageBoxButton.OK, MessageBoxImage.Information); await RefreshAsync(); });
        ...
        private async Task RunUiActionAsync(Func<Task> action)
        {
            ...
            {
                MessageBox.Show(error.Message, "Editor Context Bridge", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
    }
}
